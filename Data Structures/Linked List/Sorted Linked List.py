...
class LinkedList:
    def __init__(self, length):                     # Constructor
        ...
        self.items[length - 1] = Node(-1, None)
    ...
    def InsertNode(self, value):
        ...
        if self.items[self.start].Data > value:     # Special Case for Val smaller than Root Node
            nextNodeIndex = self.start              # Store next node index
            self.start = new                        # Start points to new node index
            self.items[new].Pointer = nextNodeIndex # New Node Points to the next node
            self.nodes = self.nodes + 1             # Increase number of nodes filled
            return 
        ...
        while True:
            ...
            nextNode = self.items[currentNode.Pointer]
            if nextNode.Data > value:                   # If next value is greater than new value
                nextNodeIndex = currentNode.Pointer     # Store next node index
                currentNode.Pointer = new               # Point to new
                self.items[new].Pointer = nextNodeIndex # New Node Points to the next node
                self.nodes = self.nodes + 1             # Increase number of nodes filled
                break
...
ll.PrintList()
print(ll.SearchFor(0))
ll.DeleteNode(ll.SearchFor(0))
print()
ll.PrintList()
ll.InsertNode(10)
print()
